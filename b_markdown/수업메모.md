...
- mp3,mp4같은 영상,음성 확장자는 ms의 저작권이므로 확장자 바꿔주는게 좋음
  - mp4 - > webm , ogg , ogv(음성지원x) 
  - mp3 - > ogg
  - safari는 mp4,mp3밖에 사용못함



### css

- display:none 의미 - 존재하지 않도록 처리하는 속성
  - screen reader에서 동작하지 않도록 처리하는 기능
  - audio/video 요소의 기능은 소리가 담겨져 있거나, 재생이 되는 요소이기 때문에 audio/video를 아예 나타나지 않아야 하는 상황. 나타나더라도 우선 muted 해줄것
...
- 속성 선택자
  ...
  >
  >요소[속성명|="값"] {} : |은 단어형식의 모든 a[href|="full"] {} (full-box 가능)
  >
  >요소[속성명~="값"] {} : ~은 분리가능한 값 |과 비슷하지만 다름 a[href~="full"] {} (full-box 불가능 full box 가능)
  >
  >요소[속성명*="값"] {} : *은 모든(전체의) a[href*="f"] {}
