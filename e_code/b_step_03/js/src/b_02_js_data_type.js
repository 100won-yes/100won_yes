...
console.log(parseInt('50.7% auto')); // 첫 글자부터 시작해서 정수 숫자로 변환가능한 것 까지만 숫자화 처리, 나머지는 버림
console.log(parseFloat('50.7% auto')); // 첫 글자부터 시작해서 실수 숫자 모두 숫자로 변환 가능한 부분까지 숫자로 처리, 나머지는 버림
console.log(Number('50.7% auto')); // 문자든 / 숫자든 관계없이 모두 숫자로 변환처리

var nan = NaN; //Not of Number
...
console.log('0~64까지의 난수', Math.random(n) * 64);
...
var fn = function(a){
  // a는 매개 변수 (argument), 인수/인자
  // return은 마지막에 작성하는 것으로, 어쨌든간에 최종 결론을 뜻함
  return 1 + a;
};
...
//              이름이 없기에 대신 이름을 할당할 수 있는 무언가를 선언 (변수 선언) : var 변수명 = function(){}