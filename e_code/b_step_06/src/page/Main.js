...
export default function Main() {
  ...
  const [slideStyle, setSlideStyle] = useState(initalStyle);
...
// useState는 값이 바뀌면 해당하는 변수에 따른 값이 재할당하기 위해 리렌더링처리
// useRef는 값이 바뀌더라도 렌더링을 다시 하지 않는다. 단순히 값만 가지고 있는다.

// useState는 변화가 일어나면 useEffect에서 감지가 됨, useState를 사용하면 렌더링이 다시 됨.
// useRef는 DOM의 변화는 일어나지 않음.
// 함수를 다시 수행하지 않거나, 화면에 뿌려주는 기능을 사용하지 않는다면 useRef를 써도 된다.
